...
export default function AccountSetting(props: { register: Register }) {
  ...
  const apiKey = global.Module.packageManager.getApikey();
          ...
          {loggedIn ? (
            <div className="plug-tg-flex plug-tg-gap-2">
              <button
                ...
                onClick={async () => {
                  await attemptLogout(global.Module);
                  triggerReload();
                }}
              >
                Logout
            ...
            <button
              onClick={async () => {
                await attemptLogin(global.Module);
                triggerReload();
              }}
            >
              Login
        ...
        {loggedIn && (
          <>
            <div className="plug-tg-flex plug-tg-w-full plug-tg-flex-col plug-tg-gap-2">
              <h3>Subscriptions:</h3>
              {global.Module.packageManager.configuration.subscriptions?.map(
                (s) => (
                  <div key={s.id}>
                    {s.name} ({s.type})
                  </div>
