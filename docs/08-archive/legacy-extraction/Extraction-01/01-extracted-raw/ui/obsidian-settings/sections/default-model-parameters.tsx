...
export default function DMPSetting(props: { register: Register }) {
  ...
  const [debouncedMaxTokens] = useDebounceValue(
    global.Module.settings.max_tokens,
    400
  );

  useEffect(() => {
    global.Module.updateStatusBar("");
  }, [debouncedMaxTokens]);
  ...
  return (
    ...
    >
      ...
      >
        <Input
          type="number"
          value={global.Module.settings.max_tokens}
          placeholder="max_tokens"
          validator={MaxTokensSchema}
          setValue={async (val) => {
            // @ts-ignore
            global.Module.settings.max_tokens = parseInt(val) || val;
      ...
      >
        <Input
          type="number"
          value={global.Module.settings.temperature}
          placeholder="temperature"
          validator={TemperatureSchema}
          setValue={async (val) => {
            // @ts-ignore
            global.Module.settings.temperature = parseFloat(val) || val;
            await global.Module.saveSettings();
            global.triggerReload();
          }}
        />
      ...
      >
        <Input
          type="number"
          value={global.Module.settings.frequency_penalty}
          placeholder="frequency_penalty"
          validator={FrequencySchema}
          setValue={async (val) => {
            // @ts-ignore
            global.Module.settings.frequency_penalty = parseFloat(val) || val;
            await global.Module.saveSettings();
            global.triggerReload();
          }}
        />
      ...
      >
        <Input
          type="number"
          placeholder="Timeout"
          value={global.Module.settings.requestTimeout}
          validator={TimeoutSchema}
          setValue={async (val) => {
            // @ts-ignore
            global.Module.settings.requestTimeout = parseInt(val) || val;
            await global.Module.saveSettings();
            global.triggerReload();
          }}
        />
      ...
      >
        <Input
          value={
            "" +
            global.Module.settings.prefix?.replaceAll(
              `
`,
              "\\n"
            )
          ...
          placeholder="Prefix"
          setValue={async (val) => {
            global.Module.settings.prefix = val.replaceAll(
              "\\n",
              `
`
            );
