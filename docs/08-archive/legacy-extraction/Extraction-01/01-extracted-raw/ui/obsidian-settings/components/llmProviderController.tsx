...
export default function LLMProviderController(props: {
  ...
  let llmList = global.Module.textGenerator.LLMRegestry.getList();
  ...
  const updateLLm = (selectedLLMId: string | undefined) => {
    ...
    global.Module.textGenerator.load();

    const llm = global.Module.textGenerator.LLMRegestry.get(selectedLLMId);
  ...
  const clone = async () => {
    ...
    // add the llm clone
    await global.Module.textGenerator.addLLMCloneInRegistry({
      id: newId,
    ...
    llmList = global.Module.textGenerator.LLMRegestry.getList();
  ...
  const del = async () => {
    ...
    await global.Module.textGenerator.deleteLLMCloneFromRegistry(selectedLLMId);
  ...
  return (
    <>
      <SettingItem
        ...
        sectionId={sectionId}
      >
        {/* {global.Module.settings.experiment ?
                <DropdownSearch
                    value={selectedLLMId}
                    ...
                    aliases={global.Module.textGenerator.LLMRegestry.UnProviderNames}
                    values={llmList}
                />
                : */}
        <Dropdown
      ...
      {!props.mini && selectedLLM && selectedLLMId && (
        <>
          ...
          {isDefaultProvider ? (
            ...
            >
              <Input
                ...
                placeholder={
                  global.Module.textGenerator.LLMRegestry.UnProviderNames[
                  selectedLLMId
                  ]
                }
                value={
                ...
                setValue={async (val) => {
                  ...
                  global.Module.settings.LLMProviderProfiles[
                    selectedLLMId
                  ].name = val;
          ...
          <ExportImportHandler
            name={
              global.Module.settings.LLMProviderProfiles[selectedLLMId]?.name
            }
            ...
            getConfig={() => {
              return {
                id: selectedLLMId,
                profile:
                  global.Module.settings.LLMProviderProfiles[selectedLLMId],
                config: global.Module.removeApikeys(global.Module.settings)
            ...
            onImport={async (data) => {
              ...
              if (d.config && "api_key" in d.config && !d.config["api_key"])
                d.config["api_key"] =
                  global.Module.settings.LLMProviderOptions?.[id]?.api_key;

              global.Module.settings.LLMProviderProfiles[id] = d.profile;
              global.Module.settings.LLMProviderOptions[id] = d.config;
              selectLLM(id);
            }}
          />
        </>
